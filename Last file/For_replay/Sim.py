...
class Sim :
	def __init__(self, screen, Mode_Sim) :
		...
		self.goal_reward = 20
		self.stable_reward = 0
		self.missing_panalty = 0
		self.collison_panalty = -10
		self.not_view_count = 0
		self.obstacle_pos = [(30, 70), (30, 120), (30, 170), (30, 220), (30, 270), (50, 100), (50, 150), (50, 200), (50, 250), (50, 250)] 
	...
	def Step(self, action) :
		...
		state = self.Make_State()
		reward = self.Check_goal() ### one frame is +1 point
	...
	def Make_obstacle(self) :
		pos = random.choice(self.obstacle_pos)
		row_size = np.random.randint(40, 60)
		obstacle = {'row' : pos[0] , 'col' : pos[1] , 'row_size' : row_size  , 'col_size' : int(row_size/2) }
	...
	def Target_move(self) :
		...
		if self.target_move :
			self.g_target_point['row'] += np.random.randint(-6, 1)
	...
	def Print_action_log(self) :
		...
		print('f :' + str(self.f_c) + ' / b : ' + str(self.b_c) + ' / r: ' + str(self.r_c) + ' / l :' + str(self.l_c))
...
class r_Sim :
	...
	def getting_fake_action(self) :
		...
		if self.t_target_point['row'] == self.t_middle_point['row'] :
			if self.t_target_point['col'] == self.t_middle_point['col'] :
				action_t = 0
			elif self.t_target_point['col'] is not self.t_middle_point['col'] :
				action_t = 2
		elif self.t_target_point['row'] < self.t_middle_point['row'] :
			if self.t_target_point['col'] == self.t_middle_point['col'] :
				action_t = 1
			elif abs(self.t_target_point['col'] - self.t_middle_point['col']) > 2 :
				if self.t_target_point['col'] >  self.t_middle_point['col'] :
					action_t = 3
				elif self.t_target_point['col'] <  self.t_middle_point['col'] :
					action_t = 4
		...
		is_out = False
		for k in range(len(self.t_obstacle_points)) :
			for i in range(self.t_obstacle_points[k]['row_end']) :
				for j in range(self.t_obstacle_points[k]['col_end']) :
					if self.t_player_point['row'] == self.t_obstacle_points[k]['row'] + i and self.t_obstacle_points[k]['col']  == self.t_obstacle_points[k]['col'] + j :
						if self.obstacle_points[k]['row'] == self.player_point['row'] :
							if self.obstacle_points[k]['col'] < self.player_point['col'] :
								action_o = 3
								is_out = True
							elif self.obstacle_points[k]['col'] > self.player_point['col'] :
								action_o = 4
								is_out = True
						elif self.obstacle_points[k]['row'] < self.player_point['row'] :
							if self.obstacle_points[k]['col'] == self.player_point['col'] :
								action_o = action_t
								is_out = True
							elif self.obstacle_points[k]['col'] > self.player_point['col'] :
								action_o = random.choice([action_t, 4])
								is_out = True
							elif self.obstacle_points[k]['col'] < self.player_point['col'] :
								action_o = random.choice([action_t, 3])
								is_out = True
						elif self.obstacle_points[k]['row'] > self.player_point['row'] :
							action_o = action_t
							is_out = True
		...
		if self.target_point['row'] == -1 :
			action_t = np.random.randint(0, 5)
			action_o = np.random.randint(0, 5)

		return [random.choice([action_t, action_o])]
