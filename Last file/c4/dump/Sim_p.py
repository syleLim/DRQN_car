...
class Sim :
	...
	def Reset(self) :
		...
		self.middle_point = {'row' : int(self.row/2), 'col' : int(self.col/2)}
		self.obstacle_points = []
	...
	def Make_State(self) :
		state = np.zeros((self.row, self.col, 3), dtype = np.int32)

		state[self.middle_point['row'], self.middle_point['col'], 2] = 1
		state[self.target_point['row'], self.target_point['col'], 1] = 1
		for obstacle in self.obstacle_points :
			for i in range(obstacle['row_size']) :
				for j in range(obstacle['col_size']) :
					if obstacle['row'] + i >= self.row :
						break
					if obstacle['col'] + j >= self.col :
						break

					state[obstacle['row'] + i, obstacle['col'] + j, 0] = 1
	...
	def Check_goal(self) :
		...
		for i in range(-5, 5) :
			for j in range(-5, 5) :
				if self.middle_point['row'] +i == self.target_point['row'] and self.middle_point['col'] + j == self.target_point['col'] :
		...
		for obstacle in self.obstacle_points :
			for i in range(obstacle['row_size'] + 30) :
				for j in range(obstacle['col_size'] + 30) :
					if self.player_point['row'] == obstacle['row'] + i - 15 and self.player_point['col'] == obstacle['col'] + j - 15 :
	...
	def Get_action(self) :
		if self.target_point['row'] < self.middle_point['row'] :
			if self.target_point['col'] > self.middle_point['col'] :
		...
		if self.target_point['row'] == self.middle_point['row'] and self.target_point['col'] == self.middle_point['col'] :
			return 0
