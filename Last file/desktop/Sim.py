...
class r_Sim :
	...
	def Update(target_point, obstacle_points) :
		self.target_point = target_point
		self.obstacle_points = obstacle_points
	...
	def getting_fake_action(self) :
		...
		is_out = False
		for k in range(lem(self.t_obstacle_points)) :
			for i in range(obstacle['row_end']) :
				for j in range(obstacle['col_end']) :
					if self.t_player_point['row'] == self.t_obstacle_points[k]['row'] + i and self.t_obstacle_points[k]['col']  == obstacle['col'] + j :
